<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" type="text/css" href="css/sideBar/whiteSideBar.css">

</head>

<body>
    <div id="sidebar">
        <div id="sidebar-background"></div>
        <div id="logo">
            <div id="logo-main" alt="Main Logo"></div>
            <div id="logo-sub"></div>
        </div>
        <div id="menu-container">
            <div class="menu-item">
                <div class="menu-icon">
                    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M34.25 3H20.5" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M34.25 10.5H20.5" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M34.25 18H20.5" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M34.25 33H20.5" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M34.25 25.5H20.5" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M1.75 4.25V14.25C1.75 14.9404 2.30964 15.5 3 15.5H13C13.6904 15.5 14.25 14.9404 14.25 14.25V4.25C14.25 3.55965 13.6904 3 13 3H3C2.30964 3 1.75 3.55965 1.75 4.25Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M1.75 21.75V31.75C1.75 32.4403 2.30964 33 3 33H13C13.6904 33 14.25 32.4403 14.25 31.75V21.75C14.25 21.0596 13.6904 20.5 13 20.5H3C2.30964 20.5 1.75 21.0596 1.75 21.75Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>

                </div>
                <div class="menu-text">대시보드</div>
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="35" height="31" viewBox="0 0 35 31" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M32.5098 1H2.50977C1.81941 1 1.25977 1.55965 1.25977 2.25V22.25C1.25977 22.9403 1.81941 23.5 2.50977 23.5H32.5098C33.2 23.5 33.7598 22.9403 33.7598 22.25V2.25C33.7598 1.55965 33.2 1 32.5098 1Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M15.0098 23.5L12.5098 29.75" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M20.0098 23.5L22.5098 29.75" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M10.0098 29.75H25.0098" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                    </svg>
                </div>
                <div class="menu-text">매장관리</div>
                <div class="arrow"><svg width="14" height="24" viewBox="0 0 14 24" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M2.85972 21.4274L10.4664 13.8208C11.3647 12.9224 11.3647 11.4524 10.4664 10.5541L2.85972 2.94744"
                        stroke="#888888" stroke-width="4.8" stroke-miterlimit="10" stroke-linecap="round"
                        stroke-linejoin="round" />
                </svg></div>
            </div>
            <div class="submenu">
                <div class="submenu-item">직원관리</div>
                <div class="submenu-item">매장 정보수정</div>
                <div class="submenu-item">명세</div>
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="32" height="34" viewBox="0 0 32 34" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M16.2598 33.5C25.0098 33.5 31.2598 30.4047 31.2598 23.5153C31.2598 16.0269 27.5098 11.0345 20.0098 7.29032L22.9598 3.49615C23.1096 3.24723 23.1909 2.96315 23.1954 2.67273C23.1999 2.3823 23.1274 2.09587 22.9852 1.84245C22.8431 1.58902 22.6364 1.37763 22.386 1.22968C22.1357 1.08173 21.8507 1.0025 21.5598 1H10.9598C10.6689 1.0025 10.3838 1.08173 10.1335 1.22968C9.88314 1.37763 9.67644 1.58902 9.53432 1.84245C9.39217 2.09587 9.31967 2.3823 9.32414 2.67273C9.32862 2.96315 9.40989 3.24723 9.55977 3.49615L12.5098 7.31527C5.00977 11.0845 1.25977 16.0768 1.25977 23.5653C1.25977 30.4047 7.50977 33.5 16.2598 33.5Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M19.7166 16.7643C19.5977 16.4279 19.4142 16.122 19.1811 15.8615C18.6852 15.3074 17.9645 14.9587 17.1624 14.9587H15.066C13.7308 14.9587 12.6484 16.0411 12.6484 17.3763C12.6484 18.5124 13.4395 19.4952 14.5494 19.738L17.7411 20.4362C18.9845 20.7082 19.8707 21.81 19.8707 23.0828C19.8707 24.5786 18.6581 25.7922 17.1624 25.7922H15.3568C14.1776 25.7922 13.1744 25.0385 12.8026 23.9866"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M16.2598 14.9584V12.25" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M16.2598 28.5002V25.7917" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                    </svg>
                </div>
                <div class="menu-text">판매기록조회</div>
                <div class="arrow"><svg width="14" height="24" viewBox="0 0 14 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M2.85972 21.4274L10.4664 13.8208C11.3647 12.9224 11.3647 11.4524 10.4664 10.5541L2.85972 2.94744"
                            stroke="#888888" stroke-width="4.8" stroke-miterlimit="10" stroke-linecap="round"
                            stroke-linejoin="round" />
                    </svg></div>

            </div>
            <div class="submenu">
                <div class="submenu-item">판매내역</div>
                <div class="submenu-item">매출</div>
            </div>
            <div id="order-menu" class="menu-item">
                <div class="menu-icon">
                    <svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M30.5811 14.7501H3.78099C3.42347 14.747 3.06944 14.8207 2.74279 14.9662C2.41617 15.1116 2.12454 15.3254 1.88759 15.5932C1.65064 15.861 1.47388 16.1764 1.36923 16.5183C1.26459 16.8602 1.2345 17.2206 1.28099 17.5751L3.10599 31.325C3.18532 31.9302 3.48334 32.4857 3.94387 32.8862C4.40439 33.287 4.99559 33.5052 5.60599 33.5H28.7061C29.3163 33.5052 29.9076 33.287 30.3681 32.8862C30.8286 32.4857 31.1266 31.9302 31.2061 31.325L33.0311 17.5751C33.0768 17.2247 33.0481 16.8686 32.9463 16.5303C32.8446 16.1919 32.6723 15.8789 32.4408 15.612C32.2093 15.345 31.9238 15.1302 31.6033 14.9816C31.2828 14.8331 30.9343 14.7541 30.5811 14.7501Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M10.9312 21V27.25" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M17.1812 21V27.25" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M23.4312 21V27.25" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M23.3809 3.6001C24.8784 3.85882 26.2504 4.59957 27.2884 5.7097C28.3264 6.8198 28.9734 8.23855 29.1309 9.7501L29.6809 14.7501"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M4.68115 14.7501L5.23115 9.75015C5.39895 8.24755 6.05045 6.84017 7.08753 5.73997C8.1246 4.63977 9.49108 3.90635 10.9812 3.65015"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M23.4312 4.125C23.4312 4.53537 23.3503 4.94175 23.1933 5.32087C23.0362 5.70002 22.8061 6.04452 22.5159 6.3347C22.2257 6.6249 21.8812 6.85508 21.502 7.01213C21.1229 7.16918 20.7165 7.25 20.3062 7.25H14.0562C13.2274 7.25 12.4325 6.92075 11.8465 6.3347C11.2604 5.74865 10.9312 4.9538 10.9312 4.125C10.9312 3.2962 11.2604 2.50135 11.8465 1.91529C12.4325 1.32924 13.2274 1 14.0562 1H20.3062C21.135 1 21.9298 1.32924 22.5159 1.91529C23.1019 2.50135 23.4312 3.2962 23.4312 4.125Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </div>
                <div id="order-text" class="menu-text">발주신청</div>
                <div class="arrow"><svg width="14" height="24" viewBox="0 0 14 24" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M2.85972 21.4274L10.4664 13.8208C11.3647 12.9224 11.3647 11.4524 10.4664 10.5541L2.85972 2.94744"
                        stroke="#888888" stroke-width="4.8" stroke-miterlimit="10" stroke-linecap="round"
                        stroke-linejoin="round" />
                </svg></div>
            </div>
            <div class="submenu">
                <div class="submenu-item">발주신청</div>
                <div class="submenu-item">발주신청 내역</div>
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <g clip-path="url(#clip0_273_6715)">
                            <path
                                d="M4.60986 21.75V33C4.60986 33.3315 4.74156 33.6495 4.97599 33.884C5.21039 34.1182 5.52834 34.25 5.85986 34.25H30.8599C31.1914 34.25 31.5094 34.1182 31.7439 33.884C31.9781 33.6495 32.1099 33.3315 32.1099 33V21.75"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M20.8599 21.75V34.25" stroke="#888888" stroke-linecap="round"
                                stroke-linejoin="round" />
                            <path d="M4.60986 25.5H20.8599" stroke="#888888" stroke-linecap="round"
                                stroke-linejoin="round" />
                            <path d="M2.10986 10.5L5.85986 1.75H30.8599L34.6099 10.5H2.10986Z" stroke="#888888"
                                stroke-linecap="round" stroke-linejoin="round" />
                            <path
                                d="M12.8099 10.5V13C12.8099 14.3261 12.2831 15.5978 11.3454 16.5355C10.4077 17.4732 9.13594 18 7.80986 18H7.10986C5.78379 18 4.51201 17.4732 3.57434 16.5355C2.63665 15.5978 2.10986 14.3261 2.10986 13V10.5"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                            <path
                                d="M23.9849 10.5V13C23.9849 14.3261 23.4581 15.5978 22.5204 16.5355C21.5827 17.4732 20.3109 18 18.9849 18H17.7349C16.4088 18 15.137 17.4732 14.1993 16.5355C13.2616 15.5978 12.7349 14.3261 12.7349 13V10.5"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                            <path
                                d="M34.6099 10.5V13C34.6099 14.3261 34.0831 15.5978 33.1454 16.5355C32.2076 17.4732 30.9359 18 29.6099 18H28.9849C27.6589 18 26.3871 17.4732 25.4493 16.5355C24.5116 15.5978 23.9849 14.3261 23.9849 13V10.5"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        </g>
                        <defs>
                            <clipPath id="clip0_273_6715">
                                <rect width="35" height="35" fill="white" transform="translate(0.859863 0.5)" />
                            </clipPath>
                        </defs>
                    </svg>
                </div>
                <div class="menu-text">제품 재고현황</div>
                
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="37" height="36" viewBox="0 0 37 36" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M14.1483 0.5H2.70602C1.96256 0.5 1.35986 1.10269 1.35986 1.84615V13.2885C1.35986 14.0319 1.96256 14.6346 2.70602 14.6346H14.1483C14.8918 14.6346 15.4945 14.0319 15.4945 13.2885V1.84615C15.4945 1.10269 14.8918 0.5 14.1483 0.5Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M35.0136 0.5H23.5713C22.8278 0.5 22.2251 1.10269 22.2251 1.84615V13.2885C22.2251 14.0319 22.8278 14.6346 23.5713 14.6346H35.0136C35.7569 14.6346 36.3597 14.0319 36.3597 13.2885V1.84615C36.3597 1.10269 35.7569 0.5 35.0136 0.5Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M14.1483 21.3654H2.70602C1.96256 21.3654 1.35986 21.9681 1.35986 22.7115V34.1538C1.35986 34.8972 1.96256 35.5 2.70602 35.5H14.1483C14.8918 35.5 15.4945 34.8972 15.4945 34.1538V22.7115C15.4945 21.9681 14.8918 21.3654 14.1483 21.3654Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M35.0136 21.3654H23.5713C22.8278 21.3654 22.2251 21.9681 22.2251 22.7115V34.1538C22.2251 34.8972 22.8278 35.5 23.5713 35.5H35.0136C35.7569 35.5 36.3597 34.8972 36.3597 34.1538V22.7115C36.3597 21.9681 35.7569 21.3654 35.0136 21.3654Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M1.35986 8.5769H15.4945" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M22.2251 8.5769H36.3597" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M22.2251 29.4423H36.3597" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M1.35986 29.4423H15.4945" stroke="#888888" stroke-linecap="round"
                            stroke-linejoin="round" />
                    </svg>
                </div>
                <div class="menu-text">메뉴조회</div>
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <g clip-path="url(#clip0_273_6816)">
                            <path
                                d="M15.8838 2.83203H31.6398C32.8118 2.83203 33.7618 3.71877 33.7618 4.81261V20.6295C33.7618 21.7234 32.8118 22.6101 31.6398 22.6101H18.7598"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                            <path
                                d="M7.88121 11.0012C10.2982 11.0012 12.2576 9.17247 12.2576 6.9166C12.2576 4.66076 10.2982 2.83203 7.88121 2.83203C5.46423 2.83203 3.50488 4.66076 3.50488 6.9166C3.50488 9.17247 5.46423 11.0012 7.88121 11.0012Z"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                            <path
                                d="M22.5098 14.5615C22.5098 13.2374 21.3597 12.1641 19.9411 12.1641H7.88209C4.22469 12.1641 1.25977 14.9313 1.25977 18.3449V23.8307H4.09789L5.04394 33.1641H10.7202L12.9004 16.9589H19.9411C21.3597 16.9589 22.5098 15.8855 22.5098 14.5615Z"
                                stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        </g>
                        <defs>
                            <clipPath id="clip0_273_6816">
                                <rect width="35" height="35" fill="white" transform="translate(0.259766 0.5)" />
                            </clipPath>
                        </defs>
                    </svg>
                </div>
                <div class="menu-text">회사</div>
                <div class="arrow"><svg width="14" height="24" viewBox="0 0 14 24" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M2.85972 21.4274L10.4664 13.8208C11.3647 12.9224 11.3647 11.4524 10.4664 10.5541L2.85972 2.94744"
                        stroke="#888888" stroke-width="4.8" stroke-miterlimit="10" stroke-linecap="round"
                        stroke-linejoin="round" />
                </svg></div>
            </div>
            <div class="submenu">
                <div class="submenu-item">회사소개</div>
                <div class="submenu-item">전체 사원조회</div>
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M11.6099 23.1501L30.8099 29.55C31.1859 29.674 31.5859 29.707 31.9771 29.646C32.3681 29.5852 32.7394 29.4322 33.0599 29.2C33.3829 28.97 33.6466 28.6665 33.8294 28.3142C34.0119 27.9622 34.1081 27.5718 34.1099 27.175V6.60005C34.1081 6.20343 34.0119 5.81293 33.8294 5.46083C33.6466 5.10873 33.3829 4.8051 33.0599 4.57505C32.7371 4.3469 32.3651 4.1985 31.9739 4.142C31.5829 4.08553 31.1839 4.12255 30.8099 4.25005L11.6099 10.6501V23.1501Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M1.60986 16.9001C1.60986 17.7209 1.77152 18.5336 2.08562 19.2919C2.39971 20.0502 2.86009 20.7392 3.44044 21.3196C4.61254 22.4917 6.20226 23.1501 7.85986 23.1501H11.6099V10.6501H7.85986C7.03911 10.6501 6.22639 10.8118 5.46809 11.1259C4.70981 11.44 4.02081 11.9004 3.44044 12.4807C2.26834 13.6528 1.60986 15.2425 1.60986 16.9001Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M7.85986 23.1501V25.6502C7.85986 26.471 8.02151 27.2837 8.33561 28.042C8.64971 28.8002 9.11009 29.4892 9.69044 30.0695C10.2708 30.65 10.9598 31.1102 11.7181 31.4245C12.4764 31.7385 13.2891 31.9002 14.1099 31.9002"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </div>
                <div class="menu-text">공지사항</div>
            </div>
            <div class="menu-item">
                <div class="menu-icon"><svg width="35" height="36" viewBox="0 0 35 36" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M17.4953 1.8797C14.5793 1.88061 11.7182 2.67242 9.21673 4.1708C6.71523 5.66917 4.667 7.818 3.29018 10.3884C1.91335 12.9588 1.25951 15.8545 1.39829 18.7671C1.53706 21.6798 2.46326 24.5002 4.07825 26.928L1.375 34.1203L10.4271 32.4832C12.6069 33.5485 14.9989 34.108 17.4249 34.12C19.851 34.132 22.2484 33.5963 24.4386 32.5528C26.6286 31.5093 28.5551 29.9848 30.0741 28.0933C31.5931 26.2015 32.6656 23.9913 33.2116 21.6275C33.7574 19.2637 33.7629 16.8071 33.2271 14.4409C32.6916 12.0747 31.6289 9.85992 30.1181 7.96165C28.6074 6.06337 26.6876 4.53058 24.502 3.47752C22.3164 2.42444 19.9213 1.87827 17.4953 1.8797Z"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M13.7754 14.2799C13.7754 13.5441 13.9936 12.8249 14.4023 12.2131C14.8111 11.6014 15.3921 11.1246 16.0718 10.843C16.7516 10.5614 17.4996 10.4878 18.2212 10.6313C18.9428 10.7748 19.6057 11.1291 20.1259 11.6494C20.6462 12.1697 21.0005 12.8325 21.144 13.5541C21.2876 14.2758 21.2139 15.0237 20.9323 15.7035C20.6508 16.3832 20.174 16.9642 19.5622 17.373C18.9504 17.7818 18.2312 17.9999 17.4954 17.9999V20.48"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M17.4999 26.6803C17.1575 26.6803 16.8799 26.4025 16.8799 26.0603C16.8799 25.7178 17.1575 25.4402 17.4999 25.4402"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M17.4995 26.6803C17.8419 26.6803 18.1195 26.4028 18.1195 26.0603C18.1195 25.7178 17.8419 25.4403 17.4995 25.4403"
                            stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </div>
                <div class="menu-text">문의사항</div>
            </div>
        </div>

    </div>
    <div id="top-bar">
        <div id="top-bar-background"></div>
        <div class="menu-management">
            <div class="menu-icons">
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M8.30769 1H1.76923C1.3444 1 1 1.3444 1 1.76923V8.30769C1 8.73252 1.3444 9.07692 1.76923 9.07692H8.30769C8.73252 9.07692 9.07692 8.73252 9.07692 8.30769V1.76923C9.07692 1.3444 8.73252 1 8.30769 1Z"
                        stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    <path
                        d="M20.2305 1H13.6921C13.2673 1 12.9229 1.3444 12.9229 1.76923V8.30769C12.9229 8.73252 13.2673 9.07692 13.6921 9.07692H20.2305C20.6553 9.07692 20.9998 8.73252 20.9998 8.30769V1.76923C20.9998 1.3444 20.6553 1 20.2305 1Z"
                        stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    <path
                        d="M8.30769 12.9231H1.76923C1.3444 12.9231 1 13.2675 1 13.6923V20.2308C1 20.6556 1.3444 21 1.76923 21H8.30769C8.73252 21 9.07692 20.6556 9.07692 20.2308V13.6923C9.07692 13.2675 8.73252 12.9231 8.30769 12.9231Z"
                        stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    <path
                        d="M20.2305 12.9231H13.6921C13.2673 12.9231 12.9229 13.2675 12.9229 13.6923V20.2308C12.9229 20.6556 13.2673 21 13.6921 21H20.2305C20.6553 21 20.9998 20.6556 20.9998 20.2308V13.6923C20.9998 13.2675 20.6553 12.9231 20.2305 12.9231Z"
                        stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M1 5.61536H9.07692" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M12.9229 5.61536H20.9998" stroke="#888888" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M12.9229 17.5385H20.9998" stroke="#888888" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M1 17.5385H9.07692" stroke="#888888" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                <div class="menu-label">메뉴관리</div>
            </div>
            <div class="page-title">메뉴조회</div>
        </div>

        <div class="announcement">
            <div class="announcement-icon">
                <svg width="36" height="35" viewBox="0 0 36 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_323_2045)">
                        <path
                            d="M11.8843 22.6501L31.0843 29.05C31.4603 29.174 31.8603 29.207 32.2515 29.146C32.6425 29.0852 33.0138 28.9322 33.3343 28.7C33.6573 28.47 33.921 28.1665 34.1038 27.8142C34.2863 27.4622 34.3825 27.0718 34.3843 26.675V6.10005C34.3825 5.70343 34.2863 5.31293 34.1038 4.96083C33.921 4.60873 33.6573 4.3051 33.3343 4.07505C33.0115 3.8469 32.6395 3.6985 32.2483 3.642C31.8573 3.58553 31.4583 3.62255 31.0843 3.75005L11.8843 10.1501V22.6501Z"
                            stroke="black" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M1.88428 16.4001C1.88428 17.2209 2.04594 18.0336 2.36003 18.7919C2.67412 19.5502 3.1345 20.2392 3.71485 20.8196C4.88695 21.9917 6.47668 22.6501 8.13428 22.6501H11.8843V10.1501H8.13428C7.31353 10.1501 6.5008 10.3118 5.7425 10.6259C4.98423 10.94 4.29523 11.4004 3.71485 11.9807C2.54276 13.1528 1.88428 14.7425 1.88428 16.4001Z"
                            stroke="black" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M8.13428 22.6501V25.1502C8.13428 25.971 8.29593 26.7837 8.61003 27.542C8.92413 28.3002 9.3845 28.9892 9.96485 29.5695C10.5452 30.15 11.2342 30.6102 11.9925 30.9245C12.7508 31.2385 13.5635 31.4002 14.3843 31.4002"
                            stroke="black" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
                    </g>
                    <defs>
                        <clipPath id="clip0_323_2045">
                            <rect width="35" height="35" fill="white" transform="translate(0.634277)" />
                        </clipPath>
                    </defs>
                </svg>
            </div>
            <div class="announcement-text">
                089 Kutch Green Apt. 448 이거진짜 여러분 방빼셔야해요
            </div>
        </div>

        <div class="branch-info">
            <div class="branch-name">회사</div>
            <div class="manager-info">
                <div class="manager-label">공장장</div>
                <div class="manager-name">황인태</div>
                <div class="manager-separator"></div>
            </div>
        </div>
    </div>
</body>


</html>